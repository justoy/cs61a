...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            elif isinstance(self.ant, QueenAnt) and self.ant.queen_count==0 :
                pass;
            else:
                self.ant=None
        else:
...
class Ant(Insect):
    ...
    def can_contain(self, other_ant):
        return self.container and (not other_ant.container) and (not self.ant)
...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 4
    min_range = 0 # for subclass
    max_range = 10 # for subclass
    ...
    def nearest_bee(self, hive):
        ...
        p = self.place
        distance = 0
        while (p is not hive) and (p!=None):
            if self.min_range <= distance <= self.max_range and p.bees:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.queen_count==0:
            ...
            p=self.place
            self.double_damage(p) # for body guard
            while p.exit:
                p=p.exit
                self.double_damage(p)
            p=self.place
            while p.entrance:
                p=p.entrance
                self.double_damage(p)
    ...
    def double_damage(self, p):
        if p.ant and not p.ant.damage_doubled:
            p.ant.damage *=2
            p.ant.damage_doubled=True
            if isinstance(p.ant, BodyguardAnt) and p.ant.ant and not p.ant.ant.damage_doubled: # for the contained ant
                p.ant.ant.damage *=2 
                p.ant.ant.damage_doubled=True
