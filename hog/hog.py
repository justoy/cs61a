...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if(num_rolls==0):
        ...
        return 1 + max(tenDigit,oneDigit)
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while(score0<goal and score1<goal):
        if(who==0):
            ...
            score1 += score
        who = other(who)
        if(is_prime(score1+score0) and score1!=score0):
            if(score0 > score1):
                score0 += score
            else:
                score1 += score
...
def max_scoring_num_rolls(dice=six_sided):
    """Return the number of dices (1 to 10) that gives the highest average turn
    ...
    max_i = 0
    for i in range(1,11):
        r=make_averaged(roll_dice)(i,dice)
        if(r>max_r):
            max_r=r
            max_i=i
    return max_i
...
def prime_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    oneDigit = opponent_score %10
    bacon = 1 + max(tenDigit,oneDigit)
    if is_prime(score+opponent_score+bacon):
        return 0 if score+bacon >opponent_score else num_rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    #the expect of average score (no special rules concerned here)
    max_r=0
    max_i = 0
    for i in range(1,11):
        r=make_averaged(roll_dice)(i,dice)
    ...
    if is_prime(score+bacon+opponent_score):
        if(score+bacon>opponent_score):
            ...
            return avg_rolls # the zero_score -> 0
        else:
            zero_score =bacon #no benefits to both players
    else:
        zero_score = bacon

    #if I may win
    if(bacon + score >= GOAL_SCORE):
        return 0

    #if the player may get 4-sided dice
    if(zero_score==avg_score):
        if(avg_score+score+opponent_score)%7==0:
            return 0

    return  0 if zero_score>avg_score else avg_rolls # Replace this statement
