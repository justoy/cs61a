...
def div_interval(x, y):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert upper_bound(y)!=lower_bound(y), "y should not spans zero!"
...
def sub_interval(x, y):
    ...
    """
    "*** YOUR CODE HERE ***"
    inverse_y = interval(-upper_bound(y),-lower_bound(y))
    return add_interval(x,inverse_y)
...
"*** YOUR CODE HERE ***"
"""par1(r1, r2) = (r1 * r2) / (r1 + r2)=(min_r1*min_r2,max_r1*max_r2)/(min_r1+min_r2,max_r1+max_r2)
                      =(max_r1*max_r2/(min_r1+min_r2), min_r1*min_r2/(max_r1+max_r2))
    par2(r1, r2) = 1 / (1/r1 + 1/r2)=1/(1/min_r1+1/min_r2, 1/max_r2+1/max_r1)
                      =(min_r1*min_r2/(min_r1+min_r2), max_r1*max_r2/(max_r1+max_r2))"""
...
def quadratic(x, a, b, c):
    ...
    extreme = -b/(2*a)
    f1,f2,f3=map(f,[lower_bound(x),upper_bound(x),extreme])
    if extreme >upper_bound(x) or extreme<lower_bound(x):
        return interval(f1,f2)
    else:
...
def polynomial(x, c):
    ...
    guesses = [lower + k*step for k in range(num_steps+1)]#guesses=[lower, upper]
    extremums=[find_zero(df,ddf,n) for n in guesses]
