...
def group_by_centroid(restaurants, centroids):
    ...
    for r in restaurants:
        _ , c=find_closest(restaurant_location(r),centroids)
        pairs.append([c,r])
...
def find_centroid(restaurants):
    ...
    "*** YOUR CODE HERE ***"
    return [mean([restaurant_location(r)[0] for r in restaurants]),
                mean([restaurant_location(r)[1] for r in restaurants])]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        "*** YOUR CODE HERE ***"
        groups = group_by_centroid(restaurants,centroids)# a list of groups of cities
        centroids = [find_centroid(group) for group in groups]
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_functions):
    ...
    "*** YOUR CODE HERE ***"
    name_restaurants_rate = {}
    reviewed_resaurants = user_reviewed_restaurants(user,restaurants)#name:restaurants

    for r in restaurants:#r is the name of a restaurant
        if r in reviewed_resaurants:
            name_restaurants_rate[r]=user_rating(user,r)
        else:
            name_restaurants_rate[r]=predictor(restaurants[r])

    return name_restaurants_rate
